...
class MyFeatureExtractor(FeatureExtractor):
    """
        ...
        - how far away the next food is
        - whether a ghost collision is imminent
        - whether a ghost (not scared) is one step away
        - whether a scared ghost is one step away
        - whether food pill will be eaten
        - how far away the next food pill is
        """
    ...
    def getFeatures(self, state, action):
        ...
        features["#-of-ghosts-1-step-away"] = sum(
            ...
            if ghostStates[ghosts.index(g)].scaredTimer <= 0)
        #
        ...
        if dist is not None:
            ...
            # will diverge wildly
            features["closest-food"] = float(dist) / (walls.width * walls.height)
        else: features["closest-food"] = 0.0

        # added: distance to closest scared ghost
        ...
        else: features["closest-scared-ghost"] = 0.0
        ...
        else: features["closest-capsule"] = 0.0


        features.divideAll(10.0)
        features.roundAll(3)

        return (features["#-of-ghosts-1-step-away"], features["eats-food"], features["closest-food"], features["closest-scared-ghost"], features["closest-capsule"])
